...
print ('%d words'%len(mnemonic.split()))
print
print mnemonic
print 

# We choose 13 random words , but Electrum DOES'T use random , it uses more entropy , XDD
#for i in range(13):

#        mnemonic = random.sample(wordlist,1)
#        random_words[i] = ''.join(mnemonic)
        

# seed for testing  >> 12QBZ51ADtzAB4RkJ7ujm7hn5DAGE6kSzj     L1RvNcpKF6Wnc5NQjiL2f5WYGZC4qZY4MyUwNTLHQ3XkSHMGdxyv
# seed = 'lyrics allow layer diet excite charge fee glory glove position seminar upon account'      
        

xprb = electrumv2_mnemonic_to_mprivkey (mnemonic)


# m/0/<n> for receiving addresses  >>>>>>>>>>>>>>>>>>>>>>   ELECTRUM uses 'wif_compressed'
# also available 'hex' , 'wif' , 'bin' , 'dec' 

for i in range(0,20):
    
    privkey = encode_privkey (bip32_extract_key (bip32_ckd(bip32_ckd(xprb, 0), i) ) ,'wif_compressed')
    
    print privtoaddr(privkey),privkey

print 

# m/1/<n> for exchange addresses

for i in range(0,6):
    
    privkey = encode_privkey (bip32_extract_key (bip32_ckd(bip32_ckd(xprb, 1), i) ) ,'wif_compressed') 
